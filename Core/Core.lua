...
RetroRuns = {
    ...
    state = {
        ...
        -- Per-run tallies for the run summary: reputation gained per
        -- faction and new appearances collected. Same lifetime as
        -- bossesKilled; rides the run record across a reload.
        runStats              = { reputation = {}, appearances = 0 },
        ...
        -- Which pool member filled each slot of a bossPool instance, by
        -- slot number. Same lifetime as bossesKilled.
        poolSlotMembers       = {},   -- [slotIndex] = bossIndex
        -- The spawnChoice member this run spawned; nil until chat names it.
        spawnedMember         = nil,
        activeStep            = nil,
        testMode              = false,
        loadedRaidKey         = nil,
...
-- A found date is held until its hour passes, never re-read on a timer: the
-- calendar's event fields turn secret inside some instances, and a re-read
-- there would trade a date in hand for a miss. A miss retries after seconds,
-- since the calendar answers empty until its data arrives.
local twEndCache, twEndMissAt = nil, 0
local TW_END_RETRY_SECONDS = 15

-- Days from `today` to the cached end day, across a month boundary.
local function TimewalkingDaysAhead(today, daysThisMonth, cache)
    if cache.month == today.month then
        return cache.day - today.monthDay
    end
    return daysThisMonth - today.monthDay + cache.day
end

-- True once the calendar's own clock is past the cached end hour.
local function TimewalkingEndPassed(today, cache)
    local nowKey = (today.year * 12 + today.month) * 31 + today.monthDay
    local endKey = (cache.year * 12 + cache.month) * 31 + cache.day
    if nowKey ~= endKey then return nowKey > endKey end
    if not cache.hour then return false end
    local nowMinute = (today.hour or 0) * 60 + (today.minute or 0)
    local endMinute = cache.hour * 60 + (cache.minute or 0)
...
function RR:GetTimewalkingEnd()
    ...
    if not today or not today.monthDay or not today.month or not today.year then
        return nil
    end

    if twEndCache then
        if not TimewalkingEndPassed(today, twEndCache) then
            local thisMonth = C_Calendar.GetMonthInfo(0)
            local daysThisMonth = (thisMonth and thisMonth.numDays) or 31
            return TimewalkingDaysAhead(today, daysThisMonth, twEndCache),
                   twEndCache.month, twEndCache.day,
    ...
    local now = GetTime and GetTime() or 0
    if twEndMissAt > 0 and (now - twEndMissAt) < TW_END_RETRY_SECONDS then
        return nil
    end
    twEndMissAt = now
        ...
        for day = firstDay, lastDay do
            ...
            for index = 1, count do
                ...
                if title and not tainted
                    ...
                    local found = {
                        year   = info and info.year or nil,
                        month  = info and info.month or nil,
                        day    = day,
                        hour   = at and at.hour or nil,
                        minute = at and at.minute or nil,
                    }
                    -- Held only when it can be aged out.
                    if found.year and found.month then twEndCache = found end
                    return ahead, found.month, day, found.hour, found.minute
                end
            end
        end
    end
...
-- The calendar delivers its events after the first read asked for them, so
-- a login's first look at the end date misses and the hourglass draws
-- untinted. Re-read once the data lands and redraw only if the answer moved.
function RR:RefreshTimewalkingEndFromCalendar()
    local hadDate = twEndCache ~= nil
    twEndMissAt = 0
    local days = self:GetTimewalkingEnd()
    if (days ~= nil) ~= hadDate and self.UI and self.UI.Update then
        if self.UI.InvalidateIdleListCache then
            self.UI.InvalidateIdleListCache()
        end
        self.UI.Update()
...
-- Inside a Timewalking run (24 dungeon, 33 raid). Queued group content,
-- so no route loads; the panel and the browser stay.
function RR:IsTimewalkingRun()
    if not self.currentRaid then return false end
    local difficultyID = self.state and self.state.currentDifficultyID
    return difficultyID == 24 or difficultyID == 33
end
...
local function CollectRaidDataIssues(scopeFilter)
    ...
    local function validateTable(tbl, tableLabel, routingOptional)
        ...
        for instanceID, raid in pairs(tbl) do
            ...
            -- altRoute needs its cue, and the cue needs a dialog line.
            if raid.altRoute and not raid.altRouteWhen then
                add("error", raidLabel, "altRoute declared without altRouteWhen")
            elseif raid.altRouteWhen and not raid.altRoute then
                add("error", raidLabel, "altRouteWhen declared without altRoute")
            elseif raid.altRouteWhen then
                local cue = raid.altRouteWhen
                if type(cue.dialog) ~= "table" or not cue.dialog.npc or not cue.dialog.match then
                    add("error", raidLabel, "altRouteWhen.dialog needs npc and match")
                end
                if cue.withoutItem ~= nil and type(cue.withoutItem) ~= "number" then
                    add("error", raidLabel, "altRouteWhen.withoutItem must be an itemID")
                end
            end
            ...
            for _, routeTable in ipairs({ raid.routing or {}, raid.altRoute or {} }) do
            for i, step in ipairs(routeTable) do
                local sp = "step " .. i .. ":"
                        ...
                        if seg.triggeredBy then
                            ...
                            elseif seg.triggeredBy.dialog then
                                ...
                                if type(dialog) ~= "table" then
                                    ...
                                    -- One line, or a list of alternatives.
                                    local alternatives = dialog[1] and dialog or { dialog }
                                    for ai, alternative in ipairs(alternatives) do
                                        local where = (#alternatives > 1)
                                            and (" triggeredBy.dialog[%d]"):format(ai)
                                            or " triggeredBy.dialog"
                                        if type(alternative) ~= "table" then
                                            add("error", raidLabel,
                                                sp .. (" segment %d%s must be a table"):format(si, where))
                                        else
                                            if not alternative.npc then
                                                add("error", raidLabel,
                                                    sp .. (" segment %d%s missing npc field"):format(si, where))
                                            end
                                            if not alternative.match then
                                                add("error", raidLabel,
                                                    sp .. (" segment %d%s missing match field"):format(si, where))
                                            end
                                        end
                                    end
                                end
                            ...
                            elseif seg.triggeredBy.item then
                                local gateItem = seg.triggeredBy.item
                                local itemOk = type(gateItem) == "number"
                                if type(gateItem) == "table" and #gateItem > 0 then
                                    itemOk = true
                                    for _, candidate in ipairs(gateItem) do
                                        if type(candidate) ~= "number" then itemOk = false end
                                    end
                                end
                                if not itemOk then
                                    add("error", raidLabel,
                                        sp .. (" segment %d triggeredBy.item must be an itemID or a list of itemIDs"):format(si))
                                end
                            elseif seg.triggeredBy.taxi ~= nil then
                                if seg.triggeredBy.taxi ~= true then
                                    add("error", raidLabel,
                                        sp .. (" segment %d triggeredBy.taxi must be true"):format(si))
                                end
                            elseif seg.triggeredBy.vehicle ~= nil then
                            ...
                            elseif seg.triggeredBy.widget then
                                if type(seg.triggeredBy.widget) ~= "number"
                                    or (seg.triggeredBy.value ~= nil
                                        and type(seg.triggeredBy.value) ~= "number") then
                                    add("error", raidLabel,
                                        sp .. (" segment %d triggeredBy.widget must be a widgetID, value a number"):format(si))
                                end
                            else
                                -- triggeredBy with no known sub-key (e.g. just empty {})
                                add("warn", raidLabel,
                                    sp .. (" segment %d triggeredBy has no recognized sub-key (expected dialog, encounter, scenario, item, taxi, or widget)"):format(si))
                            end
                        end
...
-- Numeric compare of two "major.minor.patch" strings: negative, zero or
-- positive like strcmp. A missing part reads as zero.
function RR.CompareVersions(left, right)
    local leftParts, rightParts = {}, {}
    for part in tostring(left or ""):gmatch("%d+") do leftParts[#leftParts + 1] = tonumber(part) end
    for part in tostring(right or ""):gmatch("%d+") do rightParts[#rightParts + 1] = tonumber(part) end
    for index = 1, math.max(#leftParts, #rightParts) do
        local delta = (leftParts[index] or 0) - (rightParts[index] or 0)
        if delta ~= 0 then return delta end
    end
    return 0
end

-- True when a dungeon stamped routedIn is new to this account: routed
-- after the version it last ran, or in this version when no version was
-- ever recorded. A fresh install has nothing new.
function RR.RoutedSinceVersion(routedIn, existingAccount, lastRunVersion)
    if not existingAccount or not routedIn then return false end
    if lastRunVersion then
        return RR.CompareVersions(routedIn, lastRunVersion) > 0
    end
    return routedIn == RetroRuns.VERSION
end
...
function RR:InitializeDB()
    ...
    -- routed after the version this account last ran. An account with no
    -- recorded version treats only this version's dungeons as new.
    local lastRunVersion = RetroRunsDB.lastRunVersion
    if not RetroRunsDB.seenRoutedDungeons then
        ...
        for _, dungeon in pairs(RetroRuns_DungeonData or {}) do
            if self:InstanceHasRouting(dungeon)
               and not RR.RoutedSinceVersion(dungeon.routedIn, existingAccount,
                                             lastRunVersion) then
    ...
    RetroRunsDB.lastRunVersion = RetroRuns.VERSION
...
-- Every journal walk this session, with its Lua time and the length of the
-- two frames after it. Read back by /rr warmdiag.
local ejWalkLog = {}
local ejWalkWatcher
local ejWalkOpen
local function RecordEJWalk(journalInstanceID, luaMs, encounterCount)
    local walk = {
    ...
    ejWalkLog[#ejWalkLog + 1] = walk
    ejWalkOpen = walk
    if not ejWalkWatcher then
        ejWalkWatcher = CreateFrame("Frame")
        ejWalkWatcher:SetScript("OnUpdate", function(_, elapsed)
            if not ejWalkOpen then return end
            local frameMs = elapsed * 1000
            if not ejWalkOpen.frame1 then
                ejWalkOpen.frame1 = frameMs
            else
                ejWalkOpen.frame2 = frameMs
                ejWalkOpen = nil
            end
        end)
    end
...
local function GetEJMapForJournalInstance(journalInstanceID)
    ...
    if cached then return cached end
    local walkStart = debugprofilestop()
    ...
    if EJ_SelectInstance then
        ...
        if not selectOk then
            RecordEJWalk(journalInstanceID, debugprofilestop() - walkStart, 0)
            return {}
        end
    end
    ...
    RecordEJWalk(journalInstanceID, debugprofilestop() - walkStart, count)
...
-- Report of every journal walk this session, in a copy window. Kill
-- counting reads encounter ids from the data, so the only walks expected
-- are for the instance the player is standing in.
function RR:WarmDiagDump()
    local lines = {}
    local function add(text) lines[#lines + 1] = text end
    add(("Journal walks this session: %d"):format(#ejWalkLog))
    add("(time since login / worst frame / lua / encounters / instance / caller)")
    local luaTotal, frameTotal = 0, 0
    for _, walk in ipairs(ejWalkLog) do
        local worst = math.max(walk.frame1 or 0, walk.frame2 or 0)
        luaTotal   = luaTotal + (walk.luaMs or 0)
        frameTotal = frameTotal + worst
        local name = EJ_GetInstanceInfo and EJ_GetInstanceInfo(walk.id) or "?"
        add(("  %7.1fs  %6.1fms  %5.1fms  %2d  %s (%d)"):format(
            walk.at - (RR.state.loginAt or walk.at), worst, walk.luaMs or 0,
            walk.count or 0, tostring(name), walk.id))
        add("           " .. tostring(walk.stack):gsub("%s+$", ""))
    end
    add(("Lua total: %.0fms, walk frames total: %.0fms"):format(luaTotal, frameTotal))
    self:ShowCopyWindow("WarmDiag", table.concat(lines, "\n"))
end
...
-- A boss whose lockout row lies: the server flags it complete on another
-- boss's kill (the Nexus's Anomalus shares a lockout bit with a retired
-- Frozen Commander row). The lockout is ignored for it; the kill still
-- arrives through ENCOUNTER_END, the scenario criteria and the run record.
function RR:IsBossLockoutUnreliable(boss)
    return type(boss) == "table" and boss.lockoutUnreliable == true
end
    ...
    for _, bucket in ipairs(self:GetDisplayBuckets(raid)) do
        ...
        for _, b in ipairs(raid.bosses or {}) do
            ...
            if self:BossAvailableInBucket(b, bucket)
                ...
                -- The encounter id comes from the data file, never from a
                -- journal walk: selecting an instance in the journal costs
                -- the client a long frame, and this runs for every listed
                -- instance.
                local dungeonEncID = b.dungeonEncounterID
                ...
                if dungeonEncID and not countedEncounters[dungeonEncID] then
                    ...
                    total = total + 1
                    -- A boss the lockout misreports counts only through
                    -- the live floor below.
                    local liveIds = self:IsBossLockoutUnreliable(b) and {}
                        or (liveIdsForBucket[bucket] or {})
        ...
        -- A pool instance fills its slots from a roster whose members carry
        -- no encounter of their own, so the slots are what the run counts.
        -- Members that share a real encounter were counted above.
        if raid.bossPool and raid.bossPool.slots
            and not self:PoolMembersCarryEncounters(raid) then
            total = total + #raid.bossPool.slots
        end
        ...
        if activeBucket and result[activeBucket] then
            ...
            for _, floorBucket in ipairs(floorBuckets) do
                if result[floorBucket] then
                    ...
                    if not self:PoolMembersCarryEncounters(raid) then
                        localCount = localCount + self:CountCompletePoolSlots()
                    end
            ...
            if raid.bosses then
                ...
                for _, boss in ipairs(raid.bosses) do
                    ...
                    if variant then
                        ...
                        if variant.dungeonEncounterID then
                            boss.dungeonEncounterID = variant.dungeonEncounterID
                        end
                    end
                end
...
function RR:HandleLocationChange()
    ...
    if info.instanceType ~= "raid" and info.instanceType ~= "party" then
        ...
        -- The calendar is readable again out here; a Timewalking end date
        -- an instance withheld repaints the list on the way out.
        self:RefreshTimewalkingEndFromCalendar()
        return
    end
...
RR.frame:SetScript("OnEvent", function(_, event, ...)
    ...
    elseif event == "CALENDAR_UPDATE_EVENT_LIST" then
        RR:RefreshTimewalkingEndFromCalendar()
    ...
    elseif event == "PLAYER_CONTROL_LOST" then
        -- A flight begins with control lost while on a taxi; a stun or a
    ...
    elseif event == "PLAYER_CONTROL_GAINED" then
        ...
        if RR.state.taxiInFlight then
            ...
            if RR.currentRaid and RR.state.loadedRaidKey
                and RR.state.loadedRaidKey == RR:GetRaidContextKey() then
                RR:ZoneLog("taxi landed")
                RR:AdvanceProgress("taxi-landed")
                RR:RefreshAll()
            end
        end

    elseif event == "UNIT_ENTERED_VEHICLE" then
        -- Boarding a vehicle opens a vehicle gate.
        if (...) == "player" and RR.currentRaid and RR.state.loadedRaidKey
            and RR.state.loadedRaidKey == RR:GetRaidContextKey() then
            RR:ZoneLog("vehicle entered")
    ...
    elseif event == "BAG_UPDATE_DELAYED" then
        ...
        -- bags, so the event only says "look again".
        if RR.currentRaid and RR.state.loadedRaidKey
            and RR:ActiveStepHasItemGate() then
            RR:AdvanceProgress("bag")
        end

    elseif event == "UPDATE_UI_WIDGET" then
        -- A counter widget changed; the gate reads the widget itself.
        if RR.currentRaid and RR.state.loadedRaidKey
            and RR:ActiveStepHasWidgetGate() then
            RR:RecordOpenWidgetGates()
            RR:AdvanceProgress("widget")
        end

    elseif event == "ACHIEVEMENT_EARNED" or event == "NEW_MOUNT_ADDED"
        or event == "NEW_PET_ADDED" or event == "NEW_TOY_ADDED" then
        -- A collection just changed: a boss that had something left may
        -- now have nothing, or the reverse never happens. Re-judge and let
        -- the route drop or keep its collected-skipped steps.
    ...
    elseif event == "AREA_POIS_UPDATED" then
        ...
        -- second.
        if RetroRunsMapOverlay and WorldMapFrame and WorldMapFrame:IsShown()
            and RR.currentRaid then
    ...
    elseif event == "LOOT_OPENED" then
        ...
        if not RR.state.testMode
            and RR.currentRaid
            and RR.state.loadedRaidKey == RR:GetRaidContextKey() then
            if RR:MarkBossesKilledByLoot() > 0 then
                RR:RefreshAll()
            end
        end
...
RR.frame:RegisterEvent("CALENDAR_UPDATE_EVENT_LIST")
