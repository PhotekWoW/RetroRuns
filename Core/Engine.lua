...
function RR:ActiveVariantKey()
    ...
    if self.state and self.state.activeRouteVariant == "alt"
        and raid and raid.altRoute then
        return "alt"
    end
    return "standard"
end
...
function RR:SetProgress(stepIndex, value)
    ...
    if not store then
        -- Lockout-less: the run record carries it, as it does the gates.
...
-- Which pool member filled which slot. The server's saved-instance list
-- cannot rebuild it -- pool members have no encounter -- so it rides the
-- lockout store where one exists and the run record otherwise, the same
-- split as the skips.
function RR:PersistPoolSlots()
    local store = self:GetLockoutStore(true)
    if not store then
        self:PersistRunProgress()
        return
    ...
    store.poolSlots = {}
    for slotIndex, bossIndex in pairs(self.state.poolSlotMembers or {}) do
        store.poolSlots[slotIndex] = bossIndex
    end
end

-- Which spawnChoice member this run spawned. Chat said so once and will
-- not again, so it rides the lockout store or the run record.
function RR:PersistSpawnedMember()
    local store = self:GetLockoutStore(true)
    if not store then
        self:PersistRunProgress()
        return
    end
    store.spawnedMember = self.state.spawnedMember
end
...
-- The number a UI widget shows: the first integer in its text, or a status
-- bar's value. The Oculus's "Centrifuge Constructs: 10 Remaining" is a
-- world-state line the client draws as an icon-and-text widget (544). Nil
-- when the widget is not up or its text cannot be read.
function RR:ReadWidgetNumber(widgetID)
    local manager = C_UIWidgetManager
    if not widgetID or not manager then return nil end
    local readers = {
        manager.GetIconAndTextWidgetVisualizationInfo,
        manager.GetTextWithStateWidgetVisualizationInfo,
        manager.GetStatusBarWidgetVisualizationInfo,
    }
    for _, reader in ipairs(readers) do
        local info = reader and reader(widgetID)
        if info then
            local text = info.text
            if text and not (issecretvalue and issecretvalue(text)) then
                local digits = tostring(text):match("(%d+)")
                if digits then return tonumber(digits) end
            end
            local barValue = info.barValue
            if barValue ~= nil and not (issecretvalue and issecretvalue(barValue)) then
                return tonumber(barValue)
            end
        end
    end
    return nil
...
-- triggeredBy.widget: open while the widget reads exactly `value` (0 when
-- the seg names none). A counter that has not appeared yet is not zero.
function RR:IsWidgetAtValue(widgetID, value)
    local current = self:ReadWidgetNumber(widgetID)
    if current == nil then return false end
    return current == (value or 0)
end

-- Whether a segment's gate stands open right now: no gate, a gate whose
-- event is on record, or a state check (bags, criterion, widget) that
-- passes. An item gate answers to the bags alone, fired record or not:
-- the item leaving the bags is the instance being reset under the player.
function RR:IsSegGateOpen(stepIndex, seg, segIndex)
    if not seg or not seg.triggeredBy then return true end
    if seg.triggeredBy.item then
        return self:PlayerHasItem(seg.triggeredBy.item)
    end
    if stepIndex and segIndex and self:HasTriggerFired(stepIndex, segIndex) then
        return true
    end
    if seg.triggeredBy.scenario then
        return self:IsScenarioCriterionComplete(seg.triggeredBy.scenario,
            seg.triggeredBy.quantity)
    end
    if seg.triggeredBy.widget then
        return self:IsWidgetAtValue(seg.triggeredBy.widget, seg.triggeredBy.value)
    end
...
-- True once a widget gate anywhere in the step stands open: the counter
-- it watches has landed. Markers that stood for the count dim on it.
function RR:StepWidgetGateOpen(step)
    ...
    for segIndex, seg in ipairs(step.segments) do
        if seg.triggeredBy and seg.triggeredBy.widget
            and self:IsSegGateOpen(stepIndex, seg, segIndex) then
            return true
        end
    end
    return false
...
-- A widget gate seen at its value is put on record at once, wherever the
-- player stands: the counter may land while the player is on another
-- floor's map, where the advance cannot yet move onto the gated seg, and
-- the widget may be gone by the time the player arrives.
function RR:RecordOpenWidgetGates()
    local step = self.state and self.state.activeStep
    if not step or not step.segments then return end
    local stepIndex = step.step or step.priority or 0
    for segIndex, seg in ipairs(step.segments) do
        if seg.triggeredBy and seg.triggeredBy.widget
            and not self:HasTriggerFired(stepIndex, segIndex)
            and self:IsWidgetAtValue(seg.triggeredBy.widget, seg.triggeredBy.value) then
            self:RecordTriggerFired(stepIndex, segIndex)
        end
    end
end

-- True when any segment of the active step gates on a widget, so the
-- widget listener only does work where it can matter.
function RR:ActiveStepHasWidgetGate()
    local step = self.state and self.state.activeStep
    for _, seg in ipairs(step and step.segments or {}) do
        if seg.triggeredBy and seg.triggeredBy.widget then return true end
    end
    return false
end

-- True when any segment of the active step gates on an item, so the bag
-- listener only does work where it can matter.
function RR:ActiveStepHasItemGate()
    local step = self.state and self.state.activeStep
    for _, seg in ipairs(step and step.segments or {}) do
        if seg.triggeredBy and seg.triggeredBy.item then return true end
    end
    return false
end

-- True when the player carries at least one of the item. Bags only; the
-- bank is not on the player. A list of ids means any one of them (the
-- Oculus hands out one of three drake essences).
function RR:PlayerHasItem(itemID)
    return self:CarriedItemOf(itemID) ~= nil
end

-- The first of the given item ids (one id or a list) in the bags, or nil.
function RR:CarriedItemOf(itemID)
    if not itemID then return nil end
    local getCount = (C_Item and C_Item.GetItemCount) or GetItemCount
    if not getCount then return nil end
    local candidates = (type(itemID) == "table") and itemID or { itemID }
    for _, candidate in ipairs(candidates) do
        local count = getCount(candidate)
        if not (issecretvalue and issecretvalue(count))
...
-- The item gate a segment's wording refers to: its own, else the active
-- step's, else the last one the route declared before it (the Oculus's
-- drake essence carries through every boss after Varos).
function RR:SegItemGate(seg)
    local gate = seg and seg.triggeredBy and seg.triggeredBy.item
    if not gate then
        local step = self.state and self.state.activeStep
        for _, other in ipairs(step and step.segments or {}) do
            if other.triggeredBy and other.triggeredBy.item then
                gate = other.triggeredBy.item
                break
            end
        end
    end
    if not gate then
        -- A later step naming the item again (the Oculus's drake essence
        -- carries through every boss after Varos) reads the last item
        -- gate the route declared before it.
        local routing = self.GetActiveRouting and self:GetActiveRouting()
            or (self.currentRaid and self.currentRaid.routing)
        for _, other in ipairs(routing or {}) do
            for _, otherSeg in ipairs(other.segments or {}) do
                if otherSeg.triggeredBy and otherSeg.triggeredBy.item then
                    gate = otherSeg.triggeredBy.item
                end
            end
        end
    end
...
-- `{item}` in a segment's wording names the gate item the player carries,
-- highlighted like a caret name. A gate that accepts several items names
-- whichever one is in the bags; with nothing carried the wording is left
-- as written.
function RR:FillSegItem(seg, text)
    if type(text) == "string" and text:find("{spawned}", 1, true) then
        text = self:FillSpawnedName(text)
    end
    if type(text) ~= "string" or not text:find("{item}", 1, true) then
        return text
    end
    local carried = self:CarriedItemOf(self:SegItemGate(seg))
    if not carried then return text end
    local name = (C_Item and C_Item.GetItemNameByID and C_Item.GetItemNameByID(carried))
        or (GetItemInfo and GetItemInfo(carried))
    if not name then return text end
    return (text:gsub("{item}", function() return "^" .. name .. "^" end))
end

-- `{spawned}` names the spawnChoice member this run spawned, highlighted
-- like a caret name; the set's label stands in until chat names it.
function RR:FillSpawnedName(text)
    local choice = self.currentRaid and self.currentRaid.spawnChoice
    if not choice then return text end
    local spawned = self.state.spawnedMember and self:GetBossByIndex(self.state.spawnedMember)
    local name = spawned and self:GetLocalizedBossName(spawned)
        or RR.L[choice.label or "Boss"]
    return (text:gsub("{spawned}", function() return "^" .. name .. "^" end))
end
...
function RR:ScheduleScenarioKillRetry()
    ...
    local function tick(attempt)
        ...
        if answered then
            -- A kill the ladder found is a kill like any other: written
            -- down and the step recomputed, or a run whose last boss the
            -- ladder marked stays on that boss's step instead of reading
            -- complete. Same follow-up ApplyLiveScenarioKills runs.
            if changed then
                self:PersistRunProgress()
                self:ComputeNextStep()
                self:RefreshAll()
...
function RR:PersistRunProgress()
    ...
    -- Which prisoner filled which slot only the loot witness knows, and it
    -- is not replayable, so the slots ride the record with the kills.
    local poolSlots = {}
    for slotIndex, bossIndex in pairs(self.state.poolSlotMembers or {}) do
        poolSlots[slotIndex] = bossIndex
    end
    -- Run tallies ride along so a reload keeps the summary honest.
    local stats = { reputation = {}, appearances = 0 }
    if self.state.runStats then
        for faction, amount in pairs(self.state.runStats.reputation or {}) do
            stats.reputation[faction] = amount
        end
        stats.appearances = self.state.runStats.appearances or 0
    end
    -- Segment progress too: a reload used to re-derive it from where the
    -- player stood, and a player standing where no seg of the step lives
    -- was put back on seg 1 with three gates behind them.
    local segProgress = {}
    for stepIndex, value in pairs(self.state.progress or {}) do
        segProgress[stepIndex] = value
    end
    ...
    RetroRunsDB.activeRuns[self.currentRaid.journalInstanceID or 0] = {
        ...
        skipped      = skipped,
        poolSlots    = poolSlots,
        spawnedMember = self.state.spawnedMember,
...
-- Per-run tallies: reputation per faction and new appearances. Cleared
-- wherever the kill state is, written into the run record beside it.
function RR:ResetRunStats()
    self.state.runStats = { reputation = {}, appearances = 0 }
end

function RR:RecordRunReputation(faction, amount)
    if type(faction) ~= "string" or type(amount) ~= "number" then return end
    local stats = self.state.runStats
    if not stats then self:ResetRunStats(); stats = self.state.runStats end
    stats.reputation[faction] = (stats.reputation[faction] or 0) + amount
end

function RR:RecordRunAppearances(count)
    if type(count) ~= "number" or count <= 0 then return end
    local stats = self.state.runStats
    if not stats then self:ResetRunStats(); stats = self.state.runStats end
    stats.appearances = (stats.appearances or 0) + count
    self:PersistRunProgress()
end
...
-- spawn) opens a new one. A slot ages out an hour after it was opened,
-- since the game counts instance ids from the moment each is granted.
-- The count is approximate and errs toward counting, which warns early
-- rather than late.
...
function RR:TrackInstanceEntry(info)
    ...
    -- new spawn too. The lookup starts from the last-used key, since a
    -- repeat spawn lives under a stamped one.
    local lastKey = RetroRunsDB.instanceLastSpawn[instanceKey]
    local entry = lastKey and RetroRunsDB.instanceHistory[lastKey]
    local sameBase = lastKey == spawnKey
        or (lastKey and lastKey:sub(1, #spawnKey + 1) == spawnKey .. "@")
    local idleGap = entry and (now - (entry.last or entry.create or now)) or 0
    local sameSpawn = entry and sameBase and idleGap <= INSTANCE_EMPTY_TIMEOUT
    if not sameSpawn then
        if RetroRunsDB.instanceHistory[spawnKey] then
            spawnKey = spawnKey .. "@" .. now
        end
        entry = { create = now }
        RetroRunsDB.instanceHistory[spawnKey] = entry
        lastKey = spawnKey
        local liveCount = self:GetInstanceUseCount()
        self:ZoneLog(("instance history: new slot, %d live  key=%s")
            :format(liveCount, spawnKey))
    end
    RetroRunsDB.instanceLastSpawn[instanceKey] = lastKey
    entry.last = now
end

-- The hourly book as lines for the diagnostic: every live slot with its
-- age and time left, then the last-used key per instance.
function RR:BuildInstanceHistoryLines()
    local lines = {}
    local history = RetroRunsDB and RetroRunsDB.instanceHistory
    local liveCount, freeSeconds, capLimit = self:GetInstanceUseCount()
    lines[#lines + 1] = ("live=%d/%d  oldest frees in %s  gen=%s  pendingGen=%s")
        :format(liveCount, capLimit,
                freeSeconds and (tostring(math.ceil(freeSeconds / 60)) .. "m") or "n/a",
                tostring(RetroRunsDB and RetroRunsDB.instanceHistoryGen or 1),
                tostring(RetroRunsDB and RetroRunsDB.instanceHistoryGenPending or false))
    if not history then return lines end
    local now = time()
    local keys = {}
    for key in pairs(history) do keys[#keys + 1] = key end
    table.sort(keys, function(a, b)
        return (history[a].create or 0) < (history[b].create or 0)
    end)
    for _, key in ipairs(keys) do
        local entry = history[key]
        lines[#lines + 1] = ("  %-48s created %4dm ago  touched %4dm ago")
            :format(key, math.floor((now - (entry.create or now)) / 60),
                    math.floor((now - (entry.last or entry.create or now)) / 60))
    end
    local lastSpawn = RetroRunsDB.instanceLastSpawn or {}
    for instanceKey, key in pairs(lastSpawn) do
        lines[#lines + 1] = ("  last-used %s -> %s"):format(instanceKey, key)
    end
    return lines
end
...
function RR:GetInstanceUseCount()
    ...
    local now = time()
    local liveCount, oldestOpened = 0, nil
    for key, entry in pairs(history) do
        local opened = entry.create or entry.last or 0
        if now > opened + INSTANCE_HISTORY_WINDOW then
            history[key] = nil
        else
            liveCount = liveCount + 1
            if not oldestOpened or opened < oldestOpened then
                oldestOpened = opened
            end
        end
    end
    local secondsUntilFree = oldestOpened
        and (oldestOpened + INSTANCE_HISTORY_WINDOW - now) or nil
...
function RR:RestoreRunProgress(sameSession)
    ...
    if ActiveRunMatchesCurrent(record) then
        ...
        --
        -- The signal contradicts KILLS, and only kills: a record with none
        -- (an intro event in progress -- crates found, a door opened, no
        -- boss down yet) carries gates the criteria say nothing about, and
        -- discarding it walked the player back to the start of an event
        -- the instance still remembered.
        local recordClaimsKills = next(record.killed or {}) ~= nil
            or next(record.partial or {}) ~= nil
        local freshInstance = not sameSession
            and recordClaimsKills
        ...
        elseif unproven then
            ...
            for slotIndex, bossIndex in pairs(record.poolSlots or {}) do
                self.state.poolSlotMembers[slotIndex] = bossIndex
            end
            if record.spawnedMember then
                self.state.spawnedMember = record.spawnedMember
            end
            -- Run tallies come back with the kills they describe.
            if record.stats then
                self:ResetRunStats()
                for faction, amount in pairs(record.stats.reputation or {}) do
                    self.state.runStats.reputation[faction] = amount
                end
                self.state.runStats.appearances = record.stats.appearances or 0
            end
            -- Segment progress comes back before the seeder runs; the
            -- seeder clamps it under any gate that has since closed.
            self.state.progress = self.state.progress or {}
            for stepIndex, value in pairs(record.progress or {}) do
                if (self.state.progress[stepIndex] or 0) < value then
                    self.state.progress[stepIndex] = value
                end
            end
...
function RR:RestorePersistedProgress()
    ...
    -- Named pool slots come back with their members marked dead: a member
    -- seated in a slot was looted, and the saved-instance sync cannot say
    -- so on its own.
    self.state.poolSlotMembers = self.state.poolSlotMembers or {}
    wipe(self.state.poolSlotMembers)
    for slotIndex, bossIndex in pairs(store.poolSlots or {}) do
        self.state.poolSlotMembers[slotIndex] = bossIndex
        self.state.bossesKilled[bossIndex] = true
    end
    self.state.spawnedMember = store.spawnedMember
end
...
-- One authored dialog line against a heard one: npc (optional) and a
-- substring of the text, each also tried through the locale table.
local function DialogLineMatches(dialogTrigger, eventData)
...
-- An instance may carry a second routing, `altRoute`, that the engine
-- chooses on its own when the cue in `altRouteWhen` is heard: a dialog
-- line, optionally only while an item is NOT in the bags. The Culling of
-- Stratholme's skip teleports the player past the whole intro and the
-- crier announces the waves at once; a full-run player hears that line
-- only with the disruptor in hand. Standard every run until the cue; a
-- reset starts the next run standard again. Not the raid "skip" variant,
-- which means skipped bosses and drives the skip banner and prose.
function RR:CheckAltRoute(eventData)
    local raid = self.currentRaid
    local cue = raid and raid.altRouteWhen
    if not cue or not raid.altRoute or not eventData then return false end
    if self.state.activeRouteVariant == "alt" then return false end
    if next(self.state.bossesKilled or {}) then return false end
    if cue.withoutItem and self:PlayerHasItem(cue.withoutItem) then return false end
    if not cue.dialog or not DialogLineMatches(cue.dialog, eventData) then
        return false
    end
    if self.ZoneLog then
        self:ZoneLog(("[RetroEngine] alt route: cue heard (npc=%q), switching")
            :format(tostring(eventData.npc)))
    end
    -- The standard route's progress and gates describe an intro this run
    -- never had; the alt route starts clean and seeds from here.
    wipe(self.state.progress or {})
    wipe(self.state.triggersFired or {})
    self:PersistRouteVariant("alt")
    self:ComputeNextStep()
    if self.RefreshAll then self:RefreshAll() end
    return true
end

-- triggeredBy.dialog matches an NPC dialog event's npc + text. It is one
-- line, or a list of lines any of which opens the gate -- for a cue the
-- game delivers through whichever NPC happens to speak first.
...
-- triggeredBy.item is a STATE CHECK too: it passes while the item is in
-- the player's bags, so a handed-over key or tool advances the note the
-- moment it lands.
local function TriggerMatches(seg, event, eventData)
    ...
    -- triggeredBy.taxi matches the end of a taxi flight, wherever it lands.
    if seg.triggeredBy.taxi then
    ...
    if seg.triggeredBy.vehicle then
        return event == "vehicle-entered"
            or (UnitInVehicle and UnitInVehicle("player")) or false
    end
    ...
    if seg.triggeredBy.item then
        return RR:PlayerHasItem(seg.triggeredBy.item)
    end

    -- triggeredBy.widget is a state check as well: the counter is the truth.
    if seg.triggeredBy.widget then
        return RR:IsWidgetAtValue(seg.triggeredBy.widget, seg.triggeredBy.value)
    end
    ...
    if seg.triggeredBy.dialog then
        ...
        if dialogTrigger[1] then
            for _, alternative in ipairs(dialogTrigger) do
                if DialogLineMatches(alternative, eventData) then return true end
            end
            return false
        end
        return DialogLineMatches(dialogTrigger, eventData)
    end
...
local function ComputeAdvancedProgress(segments, progress, state, event, eventData, stepIndex)
    ...
    -- A widget gate ahead that stands open: the counter it watched has
    -- landed, and the seg it opens is the one that matters now, whatever
    -- markers sit between here and there and whatever map the player is
    -- on -- like a line heard, the count is a fact about the run, and the
    -- seg's map only says where its ring draws.
    local function OpenWidgetSegAhead(fromIndex)
        for laterIdx = fromIndex + 1, segCount do
            local laterSeg = segments[laterIdx]
            if laterSeg and laterSeg.triggeredBy and laterSeg.triggeredBy.widget
                and (TriggerMatches(laterSeg, event, eventData)
                     or (stepIndex and RR:HasTriggerFired(stepIndex, laterIdx))) then
                return laterIdx
            end
        end
        return nil
    ...
    if progress >= 1 and segments[progress] then
        ...
        if not HasLeft(progressSeg, state.mapID, state.subZone) then
            ...
            if not movedDeeper then
                ...
                elseif event == "taxi-landed" then
                    -- Same narrow shape: a flight that lands on the map it
                    -- left releases stay-here only onto a seg waiting for it.
                    local nextSeg = segments[progress + 1]
                    if nextSeg and nextSeg.triggeredBy
                        and nextSeg.triggeredBy.taxi
                        and TriggerMatches(nextSeg, event, eventData)
                    then
                        gatePasses = true
                    end
                ...
                elseif event == "vehicle-entered" then
                    -- Boarding happens where the player stands.
                    local nextSeg = segments[progress + 1]
                    if nextSeg and nextSeg.triggeredBy
                        and nextSeg.triggeredBy.vehicle
                        and TriggerMatches(nextSeg, event, eventData)
                    then
                        gatePasses = true
                    end
                elseif event == "bag" then
                    -- The item landing is the release; the player is still
                    -- standing at the NPC who handed it over.
                    local nextSeg = segments[progress + 1]
                    if nextSeg and nextSeg.triggeredBy
                        and nextSeg.triggeredBy.item
                        and TriggerMatches(nextSeg, event, eventData)
                    then
                        gatePasses = true
                    end
                end
                -- A landed counter releases on any event: the last kill
                -- happens wherever the player already stands, and the
                -- record keeps the release once the widget is gone.
                if not gatePasses and OpenWidgetSegAhead(progress) then
                    gatePasses = true
                end
    ...
    while segIndex <= segCount do
        ...
        -- The event this seg waits for is proof enough on its own: a line
        -- heard or a kill seen moves the route onto the seg wherever the
        -- player stands, since the note is for right now and the seg's
        ...
        if not heardNow and seg.triggeredBy and seg.triggeredBy.widget
            and (TriggerMatches(seg, event, eventData)
                 or (stepIndex and RR:HasTriggerFired(stepIndex, segIndex))) then
            heardNow = true
        end
        if not heardNow
            and not WhenMatches(seg, state.mapID, state.subZone) then
            -- Markers drawn on another floor do not hold the walk back
            -- from a landed counter's seg on this one.
            local pulled = OpenWidgetSegAhead(segIndex)
            if not pulled then break end
            segIndex = pulled
            seg = segments[segIndex]
        end
        if not AfterSatisfied(seg, segIndex) then break end
        -- A gate whose event was already heard is open, wherever the player
        -- stood when it fired -- except an item gate, which the bags decide.
        local gateOpen = heardNow or TriggerMatches(seg, event, eventData)
            or (seg.triggeredBy and not seg.triggeredBy.item
                and stepIndex and RR:HasTriggerFired(stepIndex, segIndex))
        if not gateOpen then break end
...
function RR:AdvanceProgress(event, eventData)
    ...
    -- A heard line may be the cue that swaps the whole routing; the
    ...
    -- An event is a fact about the run, not about where the player stands:
    -- Arthas's line outside Stratholme opens the seg inside it. Record any
    -- later gate this event satisfies, so the walk can cross it once the
    -- player reaches that seg's map.
    if event == "npc-dialog" or event == "encounter-end" then
        for i = oldProgress + 1, #step.segments do
            local seg = step.segments[i]
            if seg and seg.triggeredBy
                and (seg.triggeredBy.dialog or seg.triggeredBy.encounter)
                and not self:HasTriggerFired(stepIndex, i)
                and TriggerMatches(seg, event, eventData) then
                self:RecordTriggerFired(stepIndex, i)
                if self.ZoneLog then
                    self:ZoneLog(("[RetroEngine] trigger heard ahead: step %d seg %d")
                        :format(stepIndex, i))
                end
            end
        end
...
-- A seg the route was moved onto by an EVENT (a line heard, a kill seen)
-- is shown wherever the player stands: the event is the proof its note
-- applies now, and its map only says where its points draw.
local function ComputeCurrentSeg(segments, progress, state, stepIndex)
    if not segments or #segments == 0 then return nil end
    if progress > #segments then progress = #segments end

    local atProgress = segments[progress]
    if atProgress and atProgress.note and atProgress.triggeredBy
        and (atProgress.triggeredBy.dialog or atProgress.triggeredBy.encounter
             or atProgress.triggeredBy.widget)
        and stepIndex and RR.HasTriggerFired
        and RR:HasTriggerFired(stepIndex, progress) then
        return atProgress, progress
    end
...
function RR:PickNoteSeg(step, playerMapID)
    ...
    local subZone = (GetSubZoneText and GetSubZoneText()) or ""
    local state = { mapID = mapID, subZone = subZone }
    local seg, segIdx = ComputeCurrentSeg(step.segments, progress, state, stepIndex)
...
-- Wording for a segment whose item is NOT in the bags: `noItemNote` and
-- `noItemMinNote`. The Oculus's essence can vanish mid-run, and a note
-- that says "use your {item}" has nothing to say then; this pair tells
-- the player to go and get another.
local NO_ITEM_FIELD = { note = "noItemNote", minNote = "noItemMinNote" }

function RR:ResolveSegNote(seg, field)
    if not seg or not field then return nil end
    local noItemField = NO_ITEM_FIELD[field]
    local noItemText = noItemField and seg[noItemField]
    if noItemText and noItemText ~= "" then
        local gate = self:SegItemGate(seg)
        if gate and not self:CarriedItemOf(gate) then return noItemText end
    end
    local base = seg[field]
    local variantField = SKIP_VARIANT_FIELD[field]
    if not variantField then return self:FillSegItem(seg, base) end
    local variant = seg[variantField]
    if not variant or variant == "" then return self:FillSegItem(seg, base) end
    if not seg.skipWhenBoss then return self:FillSegItem(seg, base) end
    if not self:IsBossSkipped(seg.skipWhenBoss) then return self:FillSegItem(seg, base) end
    return self:FillSegItem(seg, variant)
end
...
function RR:SeedProgress(step)
    ...
    -- A gate is open once its trigger fired this run, or -- for a
    -- scenario gate -- once the criterion already reads complete. An
    -- item gate answers to the bags alone, fired record or not: the item
    -- leaving the bags is the instance being reset under the player (the
    -- Culling's disruptor goes with the instance), and a record that
    -- says the gate fired is describing an instance that no longer exists.
    local function GateOpen(seg, i)
        return self:IsSegGateOpen(stepIndex, seg, i)
    end

    -- Gate ceiling: cap seeding at the seg before the first closed gate.
    local gateCeiling = nil
    for i, seg in ipairs(step.segments) do
        if not GateOpen(seg, i) then
            gateCeiling = i
            break
        end
    end
    ...
    -- The seed lands on the highest seg at the player's location, an open
    -- gate included: a reload after the gate fired must come back ON or
    -- past it, or the seg before it reads undone again.
    local seed = 1
    for i = upper, 1, -1 do
        local seg = step.segments[i]
        if seg and GateOpen(seg, i) then
            local segMapID = SegMapID(seg)
    ...
    -- An open gate is proof the route passed it, wherever the player now
    -- stands: a player on a map no seg of the step names still seeds at
    -- least onto the highest open gate, never back to seg 1.
    for i = upper, seed + 1, -1 do
        local seg = step.segments[i]
        if seg and seg.triggeredBy and GateOpen(seg, i) then
            seed = i
            break
        end
    end
    ...
    local oldProgress = self:GetProgress(stepIndex)
    -- Persisted progress cannot stand past a closed gate: an item gone
    -- from the bags closes its gate under whatever the record says.
    if oldProgress > upper then oldProgress = upper end
    -- Monotonic clamp: never reduce persisted progress otherwise.
    ...
    for _, step in ipairs(raid.routing or {}) do
        ...
        if step.segments then
            ...
            local seg, segIdx = ComputeCurrentSeg(step.segments, progress, state,
                step.step or step.priority or 0)
            if seg then
        ...
        add(("killed: [%s]   stamp: %s")
            :format(table.concat(killedList, ","),
                    runRecord.stamp and date("%H:%M:%S", runRecord.stamp)
                        or "none"))
        if runRecord.stats then
            local gains = {}
            for faction, amount in pairs(runRecord.stats.reputation or {}) do
                gains[#gains + 1] = ("%s %+d"):format(faction, amount)
            end
            table.sort(gains)
            add(("stats: appearances=%d  reputation=%s")
                :format(runRecord.stats.appearances or 0,
                        #gains > 0 and table.concat(gains, ", ") or "(none)"))
        end
...
function RR:DiagDump()
    ...
    divider(5, "INSTANCE HISTORY",
        "the hourly cap book -- one slot per spawned instance")
    for _, line in ipairs(self:BuildInstanceHistoryLines()) do add(line) end
...
function RR:BuildCompletionDiagLines()
    ...
    for _, b in ipairs(raid.bosses or {}) do
        ...
        local dungeonEncID = fromData or fromEJ
        ...
        elseif fromEJ then
            source = "EJ map"
        elseif self:IsPoolMember(b.index) then
            source = "pool"
        elseif b.npcID then
            -- No encounter, but the corpse is the witness.
            source = "corpse"
        else
            source = "NONE"
    ...
    for slotIndex = 1, self:GetPoolSlotCount() do
        ...
        add(("%-30s  %-12s  %-9s  %s"):format(
            (self:GetPoolSlotLabel(slotIndex) or ""):sub(1, 30),
            tostring(raid.bossPool.slots[slotIndex]),
            "slot",
            (slot.complete and "yes" or "no")
                .. (memberBoss and ("   " .. tostring(memberBoss.name)) or "")))
    end
    ...
    if raid.spawnChoice then
        local spawned = self.state.spawnedMember and self:GetBossByIndex(self.state.spawnedMember)
        add("spawnChoice: " .. (spawned and spawned.name or "not yet named"))
    end
    return out
end
