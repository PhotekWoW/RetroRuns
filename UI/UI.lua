...
do
    ...
    -- brightens from the unsupported gray through the brand cyan to white
    -- the first time the expansion is opened, then stays white. Rows are
    -- rewritten in place from a template carrying one color slot.
    UI.REVEAL_DELAY = 1.0
    UI.REVEAL_DURATION = 1.5
    UI.REVEAL_FROM = 0x8a
    UI.REVEAL_VIA = { 0x4d, 0xcc, 0xff }
    ...
    function UI.RevealHex(instance)
        ...
        if t < 0 then t = 0 end
        -- Gray to cyan over the first half, cyan to white over the second,
        -- each eased so the color swells and settles rather than steps.
        local from, to, u
        if t < 0.5 then
            from = { UI.REVEAL_FROM, UI.REVEAL_FROM, UI.REVEAL_FROM }
            to, u = UI.REVEAL_VIA, t / 0.5
        else
            from, to, u = UI.REVEAL_VIA, { 0xff, 0xff, 0xff }, (t - 0.5) / 0.5
        end
        u = u * u * (3 - 2 * u)
        local channel = {}
        for index = 1, 3 do
            channel[index] = math.floor(from[index]
                + (to[index] - from[index]) * u + 0.5)
        end
        return ("%02x%02x%02x"):format(channel[1], channel[2], channel[3])
    end
...
-- Mount marker in the header row, left of the minimize button on both the
-- panel and the minimized bar: a pink horse shown while the player is on
-- foot somewhere they can mount. On the bar it takes the search
-- shortcut's slot while it shows. Hover names the state.
panel.mountStatus = CreateFrame("Frame", nil, panel)
panel.mountStatus:SetSize(24, 24)
panel.mountStatus:EnableMouse(true)
panel.mountStatus:SetScript("OnEnter", function(self)
    if self.state ~= "foot" then return end
    local hint = RR.L["Can Mount"]
    GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
    GameTooltip:SetText(hint, 1, 1, 1)
    GameTooltip:Show()
end)
panel.mountStatus:SetScript("OnLeave", function() GameTooltip:Hide() end)
panel.mountStatus.icon = panel.mountStatus:CreateTexture(nil, "OVERLAY")
panel.mountStatus.icon:SetAllPoints()
panel.mountStatus.icon:SetTexture("Interface\\AddOns\\RetroRuns\\Media\\Icons\\HorseIcon")
panel.mountStatus.icon:SetVertexColor(C_PINK[1], C_PINK[2], C_PINK[3], 1)
-- The breath is a looping alpha animation on the icon.
-- The rhythm: solid for a second and a half, then one brief, shallow
-- dip and back, on a two-second loop.
panel.mountStatus.flash = panel.mountStatus.icon:CreateAnimationGroup()
panel.mountStatus.flash:SetLooping("REPEAT")
do
    local dip = panel.mountStatus.flash:CreateAnimation("Alpha")
    dip:SetStartDelay(1.5)
    dip:SetFromAlpha(1)
    dip:SetToAlpha(0.55)
    dip:SetDuration(0.25)
    dip:SetSmoothing("OUT")
    local rise = panel.mountStatus.flash:CreateAnimation("Alpha")
    rise:SetStartDelay(1.75)
    rise:SetFromAlpha(0.55)
    rise:SetToAlpha(1)
    rise:SetDuration(0.25)
    rise:SetSmoothing("IN")
end
panel.mountStatus:Hide()
...
local function ShowWaypointToast(anchorFrame, text, rowFS, rowRight, openRight)
    ...
    -- Offsets are measured in the panel's units, so the toast takes the
    -- panel's effective scale.
    local parentScale = (UIParent and UIParent:GetEffectiveScale()) or 1
    toast:SetScale((anchorFrame:GetEffectiveScale() or parentScale) / parentScale)
...
-- The collapsed "Boss Encounter: [!] view special note" line at the current
-- pulse phase. The heartbeat and the pulse ticker both paint it from here,
-- so the glyph breathes and the link text holds one gray.
function UI.EncounterNoteLine()
    local pulseColor = ENCOUNTER_PULSE_COLORS[encounterPulsePhase] or "|cffffff00"
    return ("|cff%s%s|r "):format(C_LABEL, RR.L["Boss Encounter:"])
        .. pulseColor .. "[!]|r |cff888888" .. RR.L["view special note"] .. "|r"
end
...
-- A slow alpha breath on a chevron glyph: full to 60% and back over 1.6s,
-- eased, always starting from full. Built on first use, and a repaint
-- never restarts one already running.
function UI.SetChevronBreathing(tex, on)
    if not tex then return end
    local anim = tex.breath
    if on then
        if not anim then
            anim = tex:CreateAnimationGroup()
            anim:SetLooping("REPEAT")
            local dim = anim:CreateAnimation("Alpha")
            dim:SetOrder(1)
            dim:SetFromAlpha(1)
            dim:SetToAlpha(0.6)
            dim:SetDuration(0.8)
            dim:SetSmoothing("IN_OUT")
            local brighten = anim:CreateAnimation("Alpha")
            brighten:SetOrder(2)
            brighten:SetFromAlpha(0.6)
...
local function BuildTravelText(step)
    ...
    -- A step whose encounter spans the walk itself opts out.
    if RR.state and RR.state.inEncounter and lastTravelText
        and not step.liveNotesInEncounter then
...
local function BuildEncounterText(step)
    ...
    local boss = RR:GetPanelBoss(step)
    ...
    elseif not RR:GetSetting("encounterExpanded") then
        ...
        headerLine = UI.EncounterNoteLine()
        clickable  = true
        headerPulsing = true
    else
        local tip = RR.L[(boss and boss.soloTip) or step.soloTip or ""]
        -- {item} names the route's gate item, as it does in a travel note.
        if RR.FillSegItem then tip = RR:FillSegItem({}, tip) end
...
-- True when the other difficulties would only repeat the current one:
-- every look still needed on the current bucket is carried on each other
...
function UI.SourcesIdenticalAcrossBuckets(boss, activeID, otherIDs, classOverride)
    ...
    if not activeID or not otherIDs or #otherIDs == 0 then return false end
    UI._equipGateExempt = UI.EquipGateExemptFor(boss.loot,
        classOverride or ActiveClassFilter())
    for _, item in ipairs(boss.loot) do
        if item.sources and ItemIsTransmogCandidate(item, classOverride) then
            local shape = ItemShape(item)
            -- A Timewalking reprint riding the row as twSource is the
            -- walk-in look, so on bucket 24 it matches any walk-in source.
            local function LookAt(diffID)
                local src = item.sources[diffID]
                if src == nil and diffID == 24 and item.twSource then
                    return true
                end
                return src
            end
            -- Whether the look at this bucket is still missing or shared.
            -- A binary row has one appearance, so its folded state stands
            -- for every bucket it is reachable at, the reprint included.
            local function NeededAt(diffID)
                local src = LookAt(diffID)
                if src == nil then return false end
                local state
                if shape == "binary" or src == true then
                    state = RR.BinaryFoldedState(item)
                else
                    state = CollectionStateForSource(src, item.id)
                end
                return state == "missing" or state == "shared"
            end
            local current = LookAt(activeID)
            local currentNeeded = NeededAt(activeID)
            for _, diffID in ipairs(otherIDs) do
                local other = LookAt(diffID)
                if currentNeeded then
                    -- A look still needed here must be reachable at every
                    -- other difficulty as the same look: a binary row is one
                    -- look wherever it is listed, any other row must carry
                    -- the very same source.
                    if other == nil then return false end
                    if shape ~= "binary" and current ~= true and other ~= true
                        and other ~= current then
                        return false
                    end
                elseif current == nil and NeededAt(diffID) then
                    -- The other side offers a look this one does not.
                    return false
                end
            end
        end
...
-- Rows from ONE boss whose own source is not collected: missing plus
-- shared. Nil on the same terms as BossAppearancesStillNeeded.
function RR:BossSourcesNotCollected(boss)
    local needed, shared = CountBossLoot(boss)
    if not needed then return nil end
    return needed + (shared or 0)
end

-- Appearances still owed across every boss of the current instance, for
-- the run summary. Bosses that offer this character nothing countable
-- contribute nothing. Nil outside an instance.
function RR:InstanceAppearancesStillNeeded()
    local raid = self.currentRaid
    if not raid or not raid.bosses then return nil end
    local total = 0
    for _, boss in ipairs(raid.bosses) do
        total = total + (CountBossLoot(boss) or 0)
    end
    return total
end
...
function UI.BuildTransmogSummaryUncached(step)
    ...
    -- Then narrowed to the buckets this boss has something in. A Heroic-only
    -- boss has no rows keyed to Normal, and an empty count read as nothing
    -- outstanding: the other-difficulty line said "Complete" about a
    -- difficulty the boss cannot be fought at. Narrowing drops the split
    -- instead. A bucket the boss carries LOOT in is kept whatever its own
    -- difficulties say, because a Timewalking row sits in a bucket those
    -- never name. An empty result means the boss is the other faction's,
    -- which this line does not speak to, so the raid's own buckets stand.
    local bossOnlyBuckets = nil
    if RR.BossAvailableInBucket then
        local bossBuckets = {}
        for _, diffID in ipairs(summaryBuckets) do
            local keep = RR:BossAvailableInBucket(boss, diffID)
            if not keep then
                for _, item in ipairs(boss.loot) do
            ...
            if keep then bossBuckets[#bossBuckets + 1] = diffID end
        end
        if #bossBuckets > 0 and #bossBuckets < #summaryBuckets then
            summaryBuckets   = bossBuckets
            bossOnlyBuckets  = bossBuckets
        end
    end

    -- Timewalking is counted on its own line and kept out of every
    -- walk-in comparison below. Folded into them, a look Timewalking cannot
    -- drop kept a Current/Other pair whose two counts read the same.
    local twBucket, walkBuckets = nil, {}
    for _, diffID in ipairs(summaryBuckets) do
        if diffID == 24 or diffID == 33 then
            twBucket = diffID
        else
            walkBuckets[#walkBuckets + 1] = diffID
        end
    end
    -- Outside a Timewalking run the line counts what the browser's TW pill
    -- marks: looks only Timewalking drops. A reprint is the walk-in look,
    -- already counted above, and its row carries no pill.
    local twNeeded, twShared, twTotal
    if twBucket and activeID ~= twBucket then
        twNeeded, twShared, twTotal = CountBossLootAcrossDifficulties(boss,
            { twBucket }, RR.PlayerClassID())
    elseif twBucket then
        twNeeded, twShared, twTotal = CountBossLootForDifficulty(boss, twBucket,
            RR.PlayerClassID())
    end
    local twDone = (not twTotal) or (twNeeded == 0 and twShared == 0)
    local allCollected = header .. "\n|cffF259C7"
        .. RR.L["All appearances collected!"] .. "|r"

    -- Inside a Timewalking run it is the current line and the walk-in
    -- difficulties are the other.
    if twBucket and activeID == twBucket then
        local n, s, t = CountBossLootAcrossDifficulties(boss, walkBuckets,
            RR.PlayerClassID())
        if not twTotal and not t then return nil end
        if twDone and ((not t) or (n == 0 and s == 0)) then return allCollected end
        local lines = { header }
        if twTotal then
            lines[#lines + 1] = ("- %s (|cff%s%s|r): %s"):format(RR.L["Current"],
                C_PINK_HEX, DIFF_LETTER[twBucket] or "TW",
                FormatStatsFragment(twNeeded, twShared))
        end
        if t then
            lines[#lines + 1] = ("- %s: %s"):format(RR.L["Other difficulties"],
                FormatStatsFragment(n, s))
        end
        return table.concat(lines, "\n")
    end

    -- Anywhere else it rides under the walk-in counts, dimmed while the
    -- week is not running.
    local twLine
    if twTotal then
        if RR:IsTimewalkingLive(RR.currentRaid) then
            twLine = ("- %s: %s"):format(RR.L["Timewalking"],
                FormatStatsFragment(twNeeded, twShared))
        else
            local dimCounts = (twNeeded == 0 and twShared == 0) and RR.L["Complete"]
                or (RR.L["Missing"] .. " (%d) " .. RR.L["Shared"] .. " (%d)")
                    :format(twNeeded, twShared)
            twLine = ("|cff9d9d9d- %s: %s|r"):format(RR.L["Timewalking"], dimCounts)
        end
    end
    local function Finish(walkText, walkDone)
        if walkDone and twDone then return allCollected end
        local out = header
        if walkText then out = out .. "\n" .. walkText end
        if twLine then out = out .. "\n" .. twLine end
        return out
    end

    -- A single walk-in bucket makes the Current-vs-Other split redundant, so
    -- the walk-in counts collapse to one rollup line. Per-item differences
    -- still show in the loot rows. A boss narrowed to one bucket rolls up over
    -- that bucket, not the modern fixed list, which its era may not use.
    if #walkBuckets <= 1 then
        local rollupBuckets = (bossOnlyBuckets and #walkBuckets == 1) and walkBuckets
            or DIFFS_FOR_SUMMARY
        local n, s, t = CountBossLootAcrossDifficulties(boss, rollupBuckets,
            RR.PlayerClassID())
        if not t and not twTotal then return nil end
        if not t then return Finish(nil, true) end
        return Finish("- " .. FormatStatsFragment(n, s), n == 0 and s == 0)
    end
    ...
    local otherIDs = {}
    for _, diffID in ipairs(walkBuckets) do
    ...
    if not activeID or not curTotal then
        local n, s, t = CountBossLootAcrossDifficulties(boss, walkBuckets, RR.PlayerClassID())
        if not t and not twTotal then return nil end
        if not t then return Finish(nil, true) end
        return Finish("- " .. FormatStatsFragment(n, s), n == 0 and s == 0)
    end
...
local function BuildTransmogSummary(step)
    ...
    local cacheKey = tostring(RR.state.loadedRaidKey or "?")
        ...
        .. ":" .. tostring(RR:GetOpenPoolSlot() or "-")
        .. ":" .. tostring(RR:IsSpawnChoicePending())
        .. ":" .. tostring(ActiveDifficulty() or "?")
        .. ":" .. tostring(UI.collectionGeneration)
        .. ":" .. tostring(RR.currentRaid and RR:IsTimewalkingLive(RR.currentRaid))
...
BuildTransmogDetail = function(stepOrCtx)
    ...
    local function FormatItemCells(item)
        ...
        if item.classes then
            ...
            -- The set a piece belongs to, when its block header names only
            -- the generation. Neutral tone: it is a grouping, not a gate.
            if item.setName and item.dungeonSet then
        ...
        local carriedUnlearned = false
        if item.bind == "BoE" or item.bagAlert then
            local carriedState = ItemSummaryState(item)
            local unlearned = carriedState == "missing" or carriedState == "shared"
    ...
    -- Rows group by set inside the block, so a boss dropping two pieces
    -- of one set shows them together.
    table.sort(setItems, function(a, b)
        if a.setName ~= b.setName then return (a.setName or "") < (b.setName or "") end
        return (a.name or "") < (b.name or "")
    end)
    ...
    if setRows and #setRows > 0 then
        ...
        -- A boss can drop pieces of several sets of one generation, so
        -- the header names the generation and each row carries its set.
        local setLabel
        if setItems[1].dungeonSet then
            setLabel = (RR.L["Dungeon Set %d"]):format(setItems[1].dungeonSet)
        else
            setLabel = RR.L[setItems[1].setName]
        end
...
function UI.PointBrowserAtCurrentInstance()
    ...
    -- While a pool slot is open the step's boss is not the one being
    -- fought; open on the prisoner last looted instead.
    local bossIndex = (RR:GetOpenPoolSlot() and RR:GetPoolBrowseBossIndex())
        or (step and step.bossIndex)
...
GetOrCreateTmogWindow = function()
    ...
    local function LayoutTmogContent(self, detail, fontSize)
        ...
        DrawSection = function(section)
            ...
            -- Collapsed over a carried, unlearned row: the chevron breathes.
            -- Open, the row's own "in your bags!" tag says it instead.
            local breathes = false
            if not self[section.flag] then
                for _, row in ipairs(section.rows or {}) do
                    if row.carried then breathes = true break end
                end
            end
            UI.SetChevronBreathing(section.toggle._chevronTex, breathes)
            section.toggle:Show()
            y = y - rowH
...
-- The mount marker's state, one answer for every surface that draws it:
--   "none"     no route active, or the client cannot say
--   "off"      mounting is not allowed where the player stands
--   "foot"     allowed, and the player is on foot
--   "mounted"  allowed, and the player is mounted
function UI.MountState()
    local routeActive = RR.currentRaid
        and RR.state.loadedRaidKey == RR:GetRaidContextKey()
    if not routeActive then return "none" end
    -- In combat there is no mounting, whatever the ground says.
    if UnitAffectingCombat and UnitAffectingCombat("player") then return "off" end
    -- Riding a vehicle (the Oculus drakes) is not being on foot, even
    -- though the summon spell may still read usable.
    if UnitInVehicle and UnitInVehicle("player") then return "off" end
    local canMount = RR.CanMountHere and RR:CanMountHere()
    if canMount == nil then return "none" end
    if not canMount then return "off" end
    if IsMounted and IsMounted() then return "mounted" end
    return "foot"
end

-- Header mount marker, one widget for the panel and the minimized bar. It
-- sits left of the minimize button at the button's scale; on the bar the
-- search shortcut yields that slot while the marker shows. Runs with the
-- minimize pass, so both layouts refresh it every heartbeat.
function UI.RefreshMountStatus()
    local slot = panel.mountStatus
    if not slot then return end
    local state = UI.MountState()
    slot.state = state
    local minimized = UI.IsMinimized and UI.IsMinimized()
    -- Only the on-foot, can-mount state shows; the others say nothing.
    local showing = (state == "foot")
    if minimized and panel.searchButton then
        panel.searchButton:SetShown(not showing)
    end
    if not showing then
        slot.flash:Stop()
        slot:Hide()
    ...
    slot:SetScale(panel.minimizeButton:GetScale() or 1)
    slot:ClearAllPoints()
    -- The expanded panel's search glass keeps the first slot when it is
    -- up (a completed run showing the instance list); the horse steps
    -- one slot further left rather than sharing the anchor.
    local glassUp = not minimized and panel.idleSearchButton
        and panel.idleSearchButton:IsShown()
    slot:SetPoint("CENTER", panel.minimizeButton, "CENTER",
        glassUp and -60 or -30, 0)
    slot.icon:SetVertexColor(C_PINK[1], C_PINK[2], C_PINK[3], 1)
    if not slot.flash:IsPlaying() then
...
-- The same TW tag on the panel's instance line inside a Timewalking run.
-- The run itself proves the week, so the tag shows even when the calendar
-- has not answered yet.
function UI.TimewalkingRunTagText(instance)
    local tag = UI.InstanceTagText(instance)
    if tag:find("TW", 1, true) then return tag end
    return "  |cff" .. UI.TIMEWALKING_TINT_OK_HEX .. "TW|r"
end
...
function UI.Update()
    ...
    if raid and loaded and RR:IsTimewalkingRun() then
        -- No route in Timewalking, so no route to name.
        panel.credit:SetText("")
    elseif raid and loaded then
        local routeLabel
        if RR:GetActiveWing() then
    ...
    if raid and loaded then
        ...
        if RR:IsTimewalkingRun() then
            raidLabel = raidLabel .. UI.TimewalkingRunTagText(raid)
        end
        ...
        elseif step then
            ...
            if routeComplete then
                ...
                -- A boss the route left out because the character has
                -- everything from it gets a line saying so, whatever the
                -- instance kind.
                local collectedNames = (RR.CollectedSkippedBossNames
                ...
                if isSkip or lockoutlessDungeon or #collectedNames > 0 then
                    ...
                    if lockoutlessDungeon or not isSkip then
                        panel.skipNote:SetText("")
                        panel.skipNote:Hide()
                    else
                ...
                if RR:IsTimewalkingRun() then
                    panel.next:SetText(("|cffff9333%s|r"):format(
                        RR.L["Routing disabled for Timewalking"]))
                elseif raid.kind == "dungeon" then
                    panel.next:SetText(("|cffff9333%s|r"):format(
...
C_Timer.NewTicker(0.1, function()
    ...
    if panel.encounter and panel.encounter.headerPulsing
        ...
        panel.encounter.header.label:SetText(UI.EncounterNoteLine())
    end
end)
...
function UI.SetBreathing(region, on)
    ...
    local anim = region.evenBreath
    if on then
        if not anim then
            anim = region:CreateAnimationGroup()
            anim:SetLooping("BOUNCE")
            local swing = anim:CreateAnimation("Alpha")
            swing:SetFromAlpha(1.0)
            swing:SetToAlpha(0.35)
            swing:SetDuration(0.8)
            swing:SetSmoothing("IN_OUT")
            region.evenBreath = anim
        end
        if not anim:IsPlaying() then anim:Play() end
    elseif anim and anim:IsPlaying() then
        anim:Stop()
